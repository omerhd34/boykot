  ...
  try {
    ...
    return {
      ...
      brands: category.brands.map((brand) => ({
        ...
        img: brand.img,
        // UPDATED: isBoycotted artık string
...
export async function getBrandBySlugs(categorySlug, brandSlug) {
  try {
    ...
    if (brand.alternativeProducts && brand.alternativeProducts.length > 0) {
      alternativeBrands = await prisma.brand.findMany({
        where: {
          slug: { in: brand.alternativeProducts },
          // UPDATED: Alternatif ürünler sadece "boykot-degil" olanlar
    ...
    return {
      ...
      alternativeProducts: alternativeBrands.map((altBrand) => ({
        id: altBrand.id,
        name: altBrand.name,
        slug: altBrand.slug,
        img: altBrand.img,
