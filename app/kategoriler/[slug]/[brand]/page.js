...
export default async function BrandDetailPage({ params }) {
 ...
 const ownerBrand = brand.owner ? await getBrandByOwnerName(brand.owner) : null;

 const founderBrand = brand.founder ? await getBrandByOwnerName(brand.founder) : null;

 const isFounderAndOwnerSame = brand.founder && brand.owner && brand.founder.trim() === brand.owner.trim();
