...
export default function AdminBrandSuggestionsPage() {
 ...
 const getStatusBadge = (status) => {
  ...
  return (
   <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${badge.className}`}>
   ...
   <div className="container mx-auto px-4">
    ...
    <div className="mb-6 flex flex-wrap gap-3">
     <button
      ...
      className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filter === "all" ? "bg-orange-500 text-white" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-200"
       }`}
     >
     ...
     <button
      ...
      className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filter === "pending" ? "bg-orange-500 text-white" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-200"
       }`}
     >
     ...
     <button
      ...
      className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filter === "approved" ? "bg-orange-500 text-white" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-200"
       }`}
     >
     ...
     <button
      ...
      className={`rounded-xl px-4 py-2 text-sm font-medium transition ${filter === "rejected" ? "bg-orange-500 text-white" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-200"
       }`}
     >
    ...
    {filteredSuggestions.length === 0 ? (
     ...
     <div className="space-y-4">
      {filteredSuggestions.map((suggestion) => (
       <div key={suggestion.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
         <div className="flex-1">
          <div className="mb-2 flex items-center gap-3">
           <h3 className="text-xl font-semibold text-slate-900">{suggestion.brandName}</h3>
        ...
        <div className="mb-4 grid gap-3 border-t border-slate-100 pt-4 text-sm">
         {suggestion.description && (
          <div>
           ...
           <span className="text-slate-600">{suggestion.description}</span>
          </div>
         )}
         <div>
